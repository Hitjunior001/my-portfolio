...
const projectDescriptions = [
  ...
  {
    "title": "FADA - Mobile",
    "description": "FADA, Support, Advocacy and Shelter Tool. A Solution FADA's main objective is to be a simple, intuitive and easy-to-use application, focused on family planning and support for women.Our mission is to implement essential features that contribute to the well-being and autonomy of users, ensuring substantial adherence and a positive impact on their lives.",
    "technologies": [
      "React Native",
      "Geolocation",
      "TypeScript",
    ...
    "githubLink": "https://github.com/Hitjunior001/app_mobile",
  },
  {
    "title": "Engine Robots - Unity Game",
    "description": "Cooperative simulation/strategy game developed in Unity where players control maintenance robots trapped in a dystopian world. The objective is to collect scrap, build complex engines, manage resources, and fulfill contracts under strict deadlines. Features include engine assembly, black market trading, robot upgrades via floppy disks, and exploration using a temporal scanner to recover ancient blueprints.",
    "technologies": [
      "Unity",
      "C#",
      "3D Modeling",
    ...
    "githubLink": "https://github.com/Hitjunior001/EngineRobots",
    "main": true,
  }
];
    ...
    >
      ...
      {projectDescriptions.map((project, i) => {
        ...
        return (
          <motion.div
            key={`main-${i}`}
            className=" border-4 border-green-500 rounded-xl overflow-hidden shadow-2xl transform hover:scale-105 transition-transform"
          ...
          >
            <div className="p-6">
              <h3 className={`text-2xl font-bold text-white mb-2 ${!buttonFixedBugs ? "glitch-text" : ""}`}>
                {project.title}
              </h3>
              <p className="text-md text-gray-200 mb-4">{project.description}</p>

              <h4 className="text-md text-green-300">Technologies:</h4>
              <div className="flex flex-wrap gap-2 mt-2">
                {project.technologies.map((tech, index) => (
                  <span
                    key={index}
                    className="text-sm text-gray-300 px-3 py-1 bg-gray-800 rounded-full"
                  >
                    {tech}
                  </span>
                ))}
